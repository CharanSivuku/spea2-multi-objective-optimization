...
### From-Scratch Implementation and Experimental Evaluation of SPEA2 on ZDT Benchmark Problems

A from-scratch Python implementation and experimental evaluation of the **Strength Pareto Evolutionary Algorithm 2 (SPEA2)** on standard **ZDT multi-objective optimization benchmark problems**.

The project implements the core SPEA2 evolutionary optimization pipeline without relying on an external multi-objective optimization framework. It includes Pareto dominance, strength calculation, fitness assignment, density estimation, environmental selection, archive truncation, tournament selection, SBX crossover, polynomial mutation, benchmark functions, performance metrics, reproducible experiments, and statistical analysis.

---

## 📌 Project Overview

Multi-objective optimization deals with problems where multiple, often conflicting, objectives must be optimized simultaneously.

Unlike single-objective optimization, where the goal is usually one optimal solution, multi-objective optimization generally produces a set of trade-off solutions known as the **Pareto-optimal set**.

This project implements **SPEA2 (Strength Pareto Evolutionary Algorithm 2)** from scratch in Python and evaluates its performance on five standard ZDT benchmark problems:

- **ZDT1** — Convex Pareto front
- **ZDT2** — Non-convex Pareto front
- **ZDT3** — Discontinuous Pareto front
- **ZDT4** — Multimodal optimization problem
- **ZDT6** — Non-uniform Pareto front

The implementation evaluates the quality of the obtained solution sets using:
...
Each benchmark is executed for **10 independent runs using seeds 1–10**.

Both individual-run results and aggregated statistical results are stored in CSV files.
...
1. Implement SPEA2 from scratch in Python.
2. Understand and implement the major components of SPEA2.
3. Implement standard ZDT benchmark functions.
4. Evaluate convergence and diversity of the obtained Pareto fronts.
5. Use multiple performance metrics instead of relying on a single metric.
6. Perform multiple independent runs to account for stochastic behavior.
7. Calculate statistical summaries across independent runs.
8. Store raw and aggregated experimental results.
9. Compare SPEA2 results with a reported PESA-II baseline.
10. Provide a reproducible experimental workflow that can be extended to other optimization problems.

---

## 🧬 What is Multi-Objective Optimization?

A multi-objective optimization problem contains multiple objective functions:

\[
\min F(x) = (f_1(x), f_2(x), ..., f_m(x))
\]

The objectives may conflict with each other. Improving one objective can therefore result in a deterioration of another objective.

Consequently, there is usually no single solution that is optimal for every objective.

Instead, the goal is to identify a set of **Pareto-optimal solutions**.

### Pareto Dominance

For minimization problems, solution `A` dominates solution `B` if:

1. `A` is no worse than `B` in every objective.
...
The set of non-dominated solutions forms the **Pareto front** in objective space.

---

## 🧠 What is SPEA2?

**SPEA2 (Strength Pareto Evolutionary Algorithm 2)** is an evolutionary algorithm for multi-objective optimization.

The algorithm maintains two sets:

- **Population** — the current candidate solutions.
- **External archive** — an elite set used to preserve high-quality solutions.

SPEA2 assigns fitness using three main concepts:

1. **Strength**
2. **Raw fitness**
3. **Density**

This allows the algorithm to simultaneously consider:

- Pareto dominance
- convergence toward good solutions
- diversity across objective space

The external archive preserves good non-dominated solutions across generations.

---

## ⚙️ SPEA2 Algorithm

The overall implementation follows this evolutionary process:
...
Return final population and archive
